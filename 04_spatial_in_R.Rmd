...
```{r read_dat}
shp <- st_read("data/shapefiles/adm2.shp")
```
...
```{r make points}
gn_hf_gps <- read_csv("data/shapefiles/GIN_HFs_microstratification.csv")

# Make the gps data into a point simple feature
gn_hf_points <- st_as_sf(gn_hf_gps, coords = c("X", "Y"), crs = 4326)
```
...
gn2 <- st_transform(gn_utm, st_crs(gn))
...
```{r load data}
annual_data <- read_csv("data/GN_routine_data_for_seasonallity_2018_2022.csv")

annual_data_sf <- shp  %>%  
                  left_join(annual_data, by = c("adm2")) 
...
> ### Task 1  {.challenge}
> - Load in the shapefile called "GN_task"
...
>  GN <- st_read("shapefiles/GN_task.shp")
>  GN
>  st_crs(GN)
>  GN <- st_transform(GN, 4326)
>  ```
> </details>
...
```{r}
population <- rast("data/population_gin_2018-10-01.tif")
population
```
...
```{r}
GPMM_extract <- extract(GPMM, vect(gn_hf_points))
...
my_breaks <- c(signif(min(gn_annual$API)-1, 2), 8000, 160000, signif(max(gn_annual$API)+1, 2))
```


```{r, message = F}
ggplot() + 
  geom_sf(data = gn_annual, aes(fill = API), colour = "grey25") + # now we need to tell ggplot an antribute of the data to use as an aesthetic
  scale_fill_viridis_c(trans = "log", breaks = my_breaks) +
  labs(title = "API, GN 20XX", fill = "") +
  coord_sf() + # deals with map projection
  guides() +
  theme_void() +
...
`ggplot() + geom_sf(data = gn_annual, aes(fill = API), colour = "grey25") +`
...
The projection and extent of our map are controlled by this function. In this case, it's not doing anything -- we could happily have omitted it from our `ggplot` call. This is because by default `ggplot2` will use the CRS of the first layer which defines one -- here `gn_annual`. This can be overridden using a `crs` argument, which accepts any valid PROJ4 string.
...
Suppose we want to add the locations of health facilities in GN to our map of API. `fakeland_hf_points` is an `sf` object, but this time consisting of points. That doesn't matter to `geom_sf`.

```{r, message = F}
fakeland_hf_gps <- read_csv("data/gn_hf_gps.csv")

# Make the gps data into a point simple feature
gn_hf_points <- st_as_sf(gn_hf_gps, coords = c("X", "Y"), crs = 4326)
# Assign some of these points to be hospitals
gn_hospitals <- sample(gn_hf_points$hf, 0.1*length(gn_hf_points$hf))

gn_hf_points <- gn_hf_points %>% 
  mutate(hf_type = if_else(hf %in% gn_hospitals, "Hospital", "Clinic"))
...
```{r, message = F}
ggplot() + 
  geom_sf(data = gn_annual, aes(fill = API), colour = "grey50", size = 0.2) + # now we need to tell ggplot an antribute of the data to use as an aesthetic
  scico::scale_fill_scico(palette = "nuuk", trans = "log", breaks = my_breaks) +
  geom_sf(data = gn_hf_points, aes(shape = hf_type), size = 2, colour = "grey20") + 
  labs(title = "API, GN 20XX", fill = "API ",
...
```{r, message = F}
admin0_regional <- st_read("shapefiles/regional/gn_admin0_region.shp")
```
...
ggplot(admin0_regional) +
  ...
  coord_sf(xlim= c(st_bbox(gn_annual)$xmin - .2, st_bbox(gn_annual)$xmax + .2),
           ylim = c(st_bbox(gn_annual)$ymin - 1, st_bbox(gn_annual)$ymax + 1)) +
...
basemap + 
  geom_sf(data = gn_annual, aes(fill = API), colour = "grey50", size = 0.2) + 
  scico::scale_fill_scico(palette = "nuuk", trans = "log", breaks = my_breaks) +
  labs(title = "API, GN 20XX", fill = "API ",
       shape = "Health Facility") +
  coord_sf(xlim= c(st_bbox(gn_annual)$xmin - .2, st_bbox(gn_annual)$xmax + .2),
